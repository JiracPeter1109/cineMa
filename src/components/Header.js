...
export const Header = () => {
  ...
  const activeClass = "block text-lg py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white"
  const inActiveClass = "block text-base py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-white dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700"

  const handleSubmit = (event) => {
    event.preventDefault();
    const queryTeam = event.target.search.value;
    event.target.reset()

    return navigate(`/search?q=${queryTeam}`)
  }


  return (
    <header>      
        <nav className="bg-white border-b-2 border-gray-200 px-2 sm:px-4 py-2.5  dark:bg-gray-900">
          <div className="container flex flex-wrap justify-between items-center mx-auto">
          <Link to="/" className="flex items-center">
              <img src={Logo} className="mr-3 h-10 sm:h-20" alt="Cinemate Logo" />
              <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">DarkDevil</span>
          </Link>
